...
export default function Search(props) {
  ...
  const { products, countProducts, categories, types, pages } = props;
    ...
    <Layout title="search">
      <div className="grid md:grid-cols-4 md:gap-5">
        <div>
          ...
          <div className="mb-3">
            <h2>Types</h2>
            <select className="w-full" value={type} onChange={brandHandler}>
              <option value="all">All</option>
              {types &&
                types.map((type) => (
          ...
          <div className="mb-2 flex items-center justify-between border-b-2 pb-2">
            ...
            <div>
              ...
              <select value={sort} onChange={sortHandler}>
                ...
                <option value="newest">Newest Meals</option>
              </select>
            </div>
          </div>
          <div>
...
export async function getServerSideProps({ query }) {
  ...
  const brandFilter = type && type !== 'all' ? { type } : {};
  const ratingFilter =
    rating && rating !== 'all'
      ? {
          rating: {
  ...
  const types = await Product.find().distinct('brand');
